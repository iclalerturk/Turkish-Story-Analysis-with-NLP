...
class MetinAnaliz:
    def __init__(self,path_hikaye, model_name="tr_core_news_trf"):
        ...
        self.characters = set()
        self.karakter_geçiş_sayacı = Counter()
        self.karakter_cümleleri = defaultdict(list)
        self.gruplar = []
    ...
    def temizle_kelime(self,kelime):
        return re.split(r"'", kelime)[0].lower()
    
    
    def tamlayici_zincir(self,token):
        tamlayicilar = []
        stack = [token]
    ...
    def get_head_word(self, tamlama):
        doc = self.nlp(tamlama)
        if len(doc) == 0:
            return None
        return doc[-1].lemma_.lower()

    def grup_ekle_mi(self, modifier, grup_modifier):
        return self.is_sublist_or_subset(modifier, grup_modifier) or self.is_sublist_or_subset(grup_modifier, modifier)

    def gruplari_olustur(self,tamlamalar_listesi):
        alt_gruplar = []
        for tamlama in tamlamalar_listesi:
            ...
            for grup in alt_gruplar:
                ...
                if self.grup_ekle_mi(modifier, grup_modifier):
                    grup.append(tamlama)
                    eklendi = True
                    break
    ...
    def benzerleri_grupla_cekirdek_kelime(self,tamlamalar, nlp):
        cekirdek_grup_dict = defaultdict(list)
        for tamlama in tamlamalar:
            cekirdek = self.get_head_word(tamlama)
        ...
        for cekirdek_lemma, tamlama_listesi in cekirdek_grup_dict.items():
            ...
            alt_gruplar = self.gruplari_olustur(tamlama_listesi)
    ...
    def grupla_benzerlik_birleştir(self,gruplar, karakter_sayilari, threshold=0.8):
        yeni_gruplar = []
        ...
        for i, grup1 in enumerate(gruplar_kopya):
            ...
            yeni_gruplar.append({
                "cekirdek": self.get_head_word(en_fazla_gecen_karakter),
    ...
    def tum_islemler(self):
        ...
        sentence = self.metin_onisleme(sentence)
        doc = self.nlp(sentence)
        # Spacy NER (kişiler)
        for ent in doc.ents:
            temiz_karakter = self.temizle_kelime(ent.text)
        ...
        for token in doc:
            if token.dep_ in ["nsubj", "nsubjpass"] and token.pos_ in ["NOUN", "PROPN", "ADJ"]:
                birlesik = self.tamlayici_zincir(token)
                if birlesik:
                ...
                # Doğrudan token'ı alıyoruz
                    ...
                    # Doğrudan token'ı alıyoruz
                    temiz_karakter = self.temizle_kelime(token.lemma_)
        ...
        gruplar = self.benzerleri_grupla_cekirdek_kelime(karakter_listesi, self.nlp)
            ...
            for sent in cümleler:
                ...
                for token in sent:
                    if token.pos_ in ["NOUN", "PROPN", "ADJ"]:
                        aday_raw = self.tamlayici_zincir(token) or token.text
                        temiz_aday = self.temizle_kelime(aday_raw)
                ...
                for pattern in patternler:
                    for aday in adaylar:
                        if re.search(pattern, aday):
                            self.karakter_cümleleri[ana_karakter].append(sent_text.strip())
                            self.karakter_geçiş_sayacı[ana_karakter] += len(re.findall(pattern, sent_lower))
                            eşleşti_mi = True
                            break
                    if eşleşti_mi:
                        break
        ...
        for karakter, sayı in self.karakter_geçiş_sayacı.items():
            print(f"{karakter}: {sayı}")

        gruplar= self.grupla_benzerlik_birleştir(gruplar, self.karakter_geçiş_sayacı, threshold=0.7500001)
        print("\nbirleştirme:")
        for i, grup in enumerate(gruplar, 1):
            print(f"Grup {i}: {grup}")
        ...
        convert_sirali_gruplar = []
        for grup in sirali_gruplar:
            karakter = grup["karakter"]
            gecis = grup["geçiş_sayısı"]
            convert_sirali_gruplar.append((karakter, gecis))
